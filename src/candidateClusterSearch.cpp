			       ...
			       bool recordIndices) 
{
  ...
  for (auto i = 0; i != vCols.size(); ++i) {
    ...
    cutPoints = getCutPointsForViableColumn(vCols[i],sortedDataMat,dThreshold,
					    subSampleThreshold,subSampleSize,subSampleIterations,twRef);
			   ...
			   bool recordIndices) 
{
  ...
  for (auto i = 0; i != vCols.size(); ++i) {
    currentDataVector = currentDataMat[vCols[i]];
    cutPoints = getCutPointsForViableColumn(vCols[i],sortedDataMat,dThreshold,
					    subSampleThreshold,subSampleSize,
					    subSampleIterations,twRef);	
...

{
  ...
  while (stillSearchingForCC) {
	...
	restrictedStackUpdate(viableCols,aCols,aRows,activeDataSubset,sortedAdmissibleSubset,baseIndex,
			      ...
			      viableColumnScores, currentPathScore, unresDataCounts,
			      subSampleThreshold,subSampleSize,subSampleIterations,mtRCCS,
			      recordIndices);
      }
      else {
