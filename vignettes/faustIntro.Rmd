...
Fast Annotation using Shape-constrained Trees (FAUST) is a new method for
automatically gating single-cell data from flow and mass cytometry experiments.
...
- The **columns** are *annotated* by the *markers* used in conducting the experiment.
...
The `FAUST` package is available on the [FredHutch GitHub](http://www.github.com/RGLab/FAUST) repository. 
...
The first parameter the user must set to use the `FAUST` method is called `pDataVar2ConcatenateSamples`.
This parameter specifies how samples in the experiment should be combined in order for `FAUST` to perform the gating.
...
or they could be combined by stimulation so that all samples under common stimulation are analyzed together; or they could be combined so all samples in the experiment are analyzed as a homogeneous group. 

Here we run `FAUST` at the sample level, so that each sample is analyzed separately.
The gating set data structure contains expermental metadata, accessed through the `pData` method.
When applied to a gating set, `pData(gs)` returns a data frame of experimental metadata.
The `FAUST` parameter `pDataVar2ConcatenateSamples` must be set to one of the column names in the `pData(gs)` data frame.
...
## `FAUST` step three: decide what channels to use.
...
## `FAUST` step four: set channel bounds.
...
## `FAUST` step five: set project path

When you run the `FAUST` pipeline, it will create a directory called `faustData` on your computer, located
at a path that you provide.

The `faustData` directory stores intermediate data used by the pipeline, plotting data produced by the pipeline, and
...
## `FAUST` step six: run FAUST to generate channel depth scores.

We've gathered all the information we need. We have the set of active channels, the lower and upper limits for annotation, 
and we have a `GatingSet`, and a project path. 

Recall that we swapped the channel and marker names in our GatingSet. We'll first save our GatingSet to our project path, and use this new path as our data source.

```{r save_gs_and_create_line_plots, warning=FALSE, eval=TRUE, echo=TRUE, message=FALSE}
save_gs(gs, path = file.path(projPath, "bcells"))
gsPath <- file.path(projPath,"bcells")
faust(
  gatingSet = gs,
  ...
  seedValue = 271828,
  #lower bound on the cluster size. If seeking rare populations, can set as low as 10.
  minClusterSize = 25,
  annotationsApproved = FALSE) # set to false before we inspect the scores plots.
```

## `FAUST` step seven: analyze the depth score line plots for each channel.
...
## `FAUST` step eight: run the annotations with the revised depth score thresholds.
...
## `FAUST` step nine: analyze the results. 
...
The last column is the "all-negative" subset. We're not interested in that here.
...
pops <- setdiff(pops,"0_0_0_0_0") #We don't want the all-negative
for (col in pops) {
  for (r in rownames(countMatrix)) {
    plotFaustGates(col, r, projPath)
  }
}
```
